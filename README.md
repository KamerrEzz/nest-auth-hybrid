...
| `POST`   | `/auth/forgot-password`     | Inicia flujo de recuperación de contraseña                         |
| `POST`   | `/auth/reset-password`      | Restablece contraseña con token                                    |
| `POST`   | `/auth/send-verification`   | Reenvía email de verificación (auth requerida)                     |
| `GET`    | `/auth/verify-email?token=` | Verifica el email con token                                        |
| `GET`    | `/health`                   | Estado del servicio (liveness)                                     |
| `GET`    | `/health/detailed`          | Estado detallado (DB + Redis)                                      |
